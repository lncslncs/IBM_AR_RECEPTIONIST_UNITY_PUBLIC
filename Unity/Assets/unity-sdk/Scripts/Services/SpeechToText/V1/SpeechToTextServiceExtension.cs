...
namespace IBM.Watson.SpeechToText.V1
{
    public partial class SpeechToTextService : BaseService
    {
        ...
        {
            Debug.Log("In STT Service StartListening Method");
            if (callback == null)
            ...
            Debug.Log("Setting _isListening to true");

            _isListening = true;
            _listenCallback = callback;
            Debug.Log("StartListening Method: Callback from STT is: " + callback);
        ...
        {
            Debug.Log("in SpeechToText.onListen method");
            bool audioSentOrEnqueued = false;
            Debug.Log("_isListening: " + _isListening);

            if (_isListening)
            {

                Debug.Log("STT is Listening...");
                if (_recordingHZ < 0)
                {
                ...
                if (!DetectSilence || _silenceDuration < _silenceCutoff)
                {
                    ...
                    else
                    {
                        ...
                        audioSentOrEnqueued = true;
                        Debug.Log("waiting for listening state from server");
        ...
        {
            Log.Debug("SpeechToText.OnListenMessage", "In OnListenMessage Method");
